...
const pkg = JSON.parse(fs.readFileSync(_path.posix.join(__dirname, '../package.json'), 'utf-8'));
...
class AdminBro {
  ...
  static bundle(src) {
    ...
    if (src[0] === '/') {
      ...
      filePath = _path.posix.join(_path.posix.dirname(m[1]), src);
    }
    ...
    const {
      ...
      dir,
      name
    } = _path.posix.parse(filePath);

    if (!extensions.find(ext => {
      const fileName = _path.posix.format({
        root,
        dir,
        name,
        ext
    ...
    AdminBro.UserComponents[componentId] = _path.posix.format({
      root,
      dir,
      name
    });
